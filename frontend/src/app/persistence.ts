...
/** api.patchConfig にそのまま渡す options (AbortSignal 等)。api/ が .ts になったら実体の型に置き換える。 */
type PatchOpts = Parameters<typeof api.patchConfig>[2];
/** config の部分更新。キーは config のトップレベル項目名 */
type ConfigPatch = Record<string, unknown>;

const CONFIG_DEBOUNCE_MS = 600;

let currentSid: string | null = null;
const _pending = new Map<string, ConfigPatch>();   // sid -> partial patch object (mutable)
const _timers = new Map<string, ReturnType<typeof setTimeout>>();    // sid -> setTimeout id
const _chain = new Map<string, Promise<void>>();     // sid -> Promise (直近の flush)

function ensurePending(sid: string): ConfigPatch {
  let p = _pending.get(sid);
  if (!p) { p = {}; _pending.set(sid, p); }
  return p;
}

function clearDebounce(sid: string) {
...
async function doFlush(sid: string, opts?: PatchOpts): Promise<void> {
  while (true) {
...
function flushConfigForSid(sid: string | null, opts?: PatchOpts): Promise<void> {
...
export function setCurrentSourceId(sid: string | null) {
...
export function queueConfigPatch(patch: ConfigPatch) {
...
export async function flushConfigNow(opts?: PatchOpts): Promise<void> {
...
export function clearPendingConfigKeys(keys: Iterable<string>) {
  if (!currentSid) return;
