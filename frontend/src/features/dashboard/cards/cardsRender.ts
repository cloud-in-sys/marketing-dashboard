...
import { aggregate, metricUsesHierarchy } from '@aggregate/aggregate.ts';
import { formatMetricValue } from '../charts/chart.ts';
import { getBackendCardAgg } from '@aggregate/aggregateCache.ts';
import { resolveDateFilter } from '@filters/dateFilter.ts';

// ===== KPIカード =====
// 期間フィルタ用のヘルパー: 日付フィルタの field を特定してその月一覧を抽出
function getCardDateField(): string {
  return resolveDateFilter().field;
}
// 日付値の YYYY-MM 部分を取得 (時刻や '/' 区切りも吸収)
function rowMonth(v: any): string {
  if (v == null) return '';
  return String(v).slice(0, 10).replace(/\//g, '-').slice(0, 7);
}
function getMonthsAvailable(rows: any[], field: string): string[] {
...
function isFilterRangeOpen(): boolean {
...
function applyCardFilter(rows: any[], card: CardConfig): any[] {
  ...
  if (mode === 'current_month') {
    ...
    return source.filter((r: any) => rowMonth(r[field]) === target);
  }
...
export function renderCards(rows: any[]) {
...
export function openCardSettings(cardId: number) {
