...
import { S, saveState } from '@app/state.ts';
import { openCardSettings, closeCardSettings, renderCardSettingsPanel } from './cardsRender.ts';
import { canEditCurrentTab } from '@app/auth.ts';
...
document.getElementById('cards-grid')!.addEventListener('click', e => {
  ...
  if ((e.target as HTMLElement).closest('[data-card-role="remove"]')) {
    S.CARDS = S.CARDS.filter((c) => c.id !== id);
    if (S.CARD_SETTINGS_ID === id) closeCardSettings();
    emit('render');
    return;
  }
  if ((e.target as HTMLElement).closest('[data-card-role="settings"]')) {
    openCardSettings(id);
  }
});
// インライン名前変更
document.getElementById('cards-grid')!.addEventListener('input', e => {
  const role = (e.target as HTMLInputElement).dataset.cardRole;
  ...
  if (!c) return;
  c.label = (e.target as HTMLInputElement).value;
  // ライブ再描画は重いので、フォーカス保持のため再描画はスキップ。値は state にのみ保存。
  saveState();
});
function onCardPanelChange(e: Event) {
  const role = (e.target as HTMLInputElement).dataset.cardPanelRole;
  if (!role) return;
  const c = S.CARDS.find((x) => x.id === S.CARD_SETTINGS_ID);
  if (!c) return;
  // role は data-card-panel-role 由来の動的キー (label / size / metric / filterMode / *Color 等)。
  // いずれも CardConfig の string フィールドだが、任意 string での添字書き込みは
  // 型で表現しきれないため、この 1 箇所だけキャストする (実行時挙動は c[role] と同一)。
  (c as Record<string, any>)[role] = (e.target as HTMLInputElement).value;
  emit('render');
}
...
document.getElementById('card-settings-body')!.addEventListener('click', e => {
  const btn = (e.target as HTMLElement).closest('[data-card-panel-role="resetColors"]');
  if (!btn) return;
  const c = S.CARDS.find((x) => x.id === S.CARD_SETTINGS_ID);
  if (!c) return;
  delete c.bgColor;
...
// ===== CARD DRAG REORDER =====
let CARD_DRAG_ID: number | null = null;
...
cardsGrid.addEventListener('dragstart', e => {
  ...
  const card = (e.target as HTMLElement).closest('.kpi-card') as HTMLElement | null;
  if (!card) return;
