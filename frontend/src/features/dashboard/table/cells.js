...
import { fmt, escapeHtml } from '../../../shared/utils/utils.js';
...
import { buildCellStyle } from './tableSettings.js';
...
export function thresholdClass(metricKey, value) {
...
export function metricCellHtml(m, agg, opts = {}, groupVals = null, extraClass = '') {
...
export function buildMetricCells(agg, metrics, opts = {}, groupVals = null) {
  return metrics.map(m => metricCellHtml(m, agg, opts, groupVals)).join('');
}
// URL 安全判定: link は http/https のみ、image はそれに加えて data:image/* を許容。
// `javascript:` `data:text/html` 等の危険スキームは弾く。
export function isSafeLink(s) {
  if (typeof s !== 'string' || !s) return false;
  return /^https?:\/\//i.test(s.trim());
}
export function isSafeImageSrc(s) {
  if (typeof s !== 'string' || !s) return false;
  const t = s.trim();
  return /^https?:\/\//i.test(t) || /^data:image\/[a-zA-Z0-9+.-]+;/.test(t);
}

export function dimCellHtml(dimKey, value, extraClasses = '', innerHtml = null, dimIdx = null, opts = {}) {
