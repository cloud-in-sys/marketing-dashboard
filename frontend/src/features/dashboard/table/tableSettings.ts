...
function ensureSortList(cfg: any): any[] {
...
export function subtotalAt(cfg: any, depth: number): boolean {
...
function setField(bucket: string, key: string | null, field: string, value: any) {
...
function decoControls(style: any, roleNs: string): string {
...
function alignControls(style: any, roleNs: string): string {
  const cur = style.align || '';
  const opt = (v: string, label: string, title: string) => `
...
function vAlignControls(style: any, roleNs: string): string {
...
function colorField(label: string, role: string, val?: string, fallback?: string): string {
  const isDefault = !val;
...
// 文字色 / 背景色 のペア
function colorControls(style: any, roleNs: string, defaults = { color: '#334155', bgColor: '#ffffff' }): string {
  return colorField('文字', `${roleNs}.color`, style.color, defaults.color)
       + colorField('背景', `${roleNs}.bgColor`, style.bgColor, defaults.bgColor);
}

function syncColorFieldUi(el: HTMLElement, hasValue: boolean) {
  ...
  const clear = wrap.querySelector('.table-settings-clear') as HTMLButtonElement | null;
...
const _openSections = new Set<number>();
function captureOpenSections() {
  _openSections.clear();
  document.querySelectorAll<HTMLDetailsElement>('#table-settings-body details.table-settings-section').forEach((d, i) => {
...
export function renderTableSettingsPanel() {
  ...
  const dimCols = (S.SELECTED_DIMS || []).map((k: string) => ({ key: 'dim:' + k, label: dimLabel(k), kind: 'dim' }));
  const metCols = (S.METRIC_DEFS || []).filter((m: any) => (S.SELECTED_METRICS || []).includes(m.key))
    .map((m: any) => ({ key: m.key, label: m.label, kind: 'metric' }));
  const columns = [...dimCols, ...metCols];

  const colStyleRow = (c: any, bucket: string) => {
  ...
  const t = cfg.table;
  const defField = (label: string, role: string, val?: string, fallback = '') => colorField(label, role, val, fallback);
...
function filterRow(c: any, cfg: any): string {
...
// プルダウンから列を選ぶだけで動き出す。
function sortBlock(columns: any[], cfg: any): string {
  ...
  return `
    ...
    <div style="margin-top:10px;display:flex;gap:8px">
      ...
      ${list.some((it: any) => it && it.col) ? '<button type="button" class="link-btn" data-table-role="clear:sort" title="並び替えを全てリセット">すべてリセット</button>' : ''}
    </div>
  `;
}
...
function onPanelChange(e: Event) {
  // 元から el という別名変数がある (実行時コードは変えていない)
...
function onPanelClick(e: Event) {
  // 並び替えを追加
  const addS = (e.target as HTMLElement).closest('[data-table-role="add-sort"]');
  if (addS) {
    const cfg = ensureConfig();
    const list = ensureSortList(cfg);
  ...
  const rmS = (e.target as HTMLElement).closest('[data-table-role^="remove-sort:"]') as HTMLElement | null;
  if (rmS) {
  ...
  const fz = (e.target as HTMLElement).closest('[data-table-role^="filter-zero:"]') as HTMLElement | null;
  if (fz) {
  ...
  // 単一フィールドクリア (× ボタン)
  const clr = (e.target as HTMLElement).closest('[data-table-role^="clear:"]') as HTMLButtonElement | null;
  if (clr && !clr.disabled) {
  ...
  const btn = (e.target as HTMLElement).closest('[data-table-role^="reset"]') as HTMLElement | null;
  if (!btn) return;
...
function styleToCss(s: any, includeDeco = true): string {
  if (!s) return '';
  const parts: string[] = [];
...
export function buildCellStyle(key: string): string {
  return styleToCss(S.TABLE_CONFIG?.styles?.[key]);
}
// 列ヘッダー (th) のスタイル: 列ごとのヘッダー設定のみ (全体ヘッダー設定は廃止)。
export function buildHeaderCellStyle(key: string): string {
...
export function buildTableStyle(): string {
