...
import { evalFormula, baseMetricKeys, derivedMetricKeys } from '@aggregate/aggregate.ts';
import { S, DEFAULT_FORMULAS } from '@app/state.ts';
import { getBackendGroups } from '@aggregate/aggregateCache.ts';
...
export function parseSparkline(formula: any): SparklineConfig | null {
...
export function getSparklineConfig(metricKey: string): SparklineConfig | null {
...
function sanitizeColor(v: any): string | null {
...
export function renderSparklineSVG(series: any[] | null, opts: Record<string, any> = {}, width = 100, height = 28): string {
...
function clampSize(v: any, fallback: number, min: number, max: number): number {
  const n = Number(v);
...
export function getSparklineSeries(groupKey: string): any[] | null {
  return (S.SPARKLINE_SERIES instanceof Map ? S.SPARKLINE_SERIES.get(groupKey) : null) || null;
}
function makeGroupSeriesKey(vals: any[]): string {
  return vals.map((v: any) => String(v ?? '')).join('');
}
...
const _globalMaxMap = new Map<string, number>(); // Map<`${metricKey}:${depth}`, number>
const _pathDepthMap = new Map<string, number>(); // Map<pathKey, depth>
function recomputeGlobalMax() {
  ...
  for (const sm of sparkMetrics) {
    const depthMax = new Map<number, number>(); // depth -> max
    for (const [pathKey, arr] of series.entries()) {
      ...
      const rowAgg = sumBucketAggs(arr.map((b: any) => b.agg));
...
function getVisibleSparklineMetrics(): { key: string; cfg: SparklineConfig }[] {
  return (S.SELECTED_METRICS || [])
    .map((k: string) => ({ key: k, cfg: getSparklineConfig(k) }))
    .filter(x => x.cfg) as { key: string; cfg: SparklineConfig }[];
}
...
export function rowKeyForSparkline(rowVals: any[]): string {
  if (_seriesTimeIdxInCurrentDims < 0) return makeGroupSeriesKey(rowVals);
  return makeGroupSeriesKey(rowVals.filter((_: any, i: number) => i !== _seriesTimeIdxInCurrentDims));
}
// 行の depth (= series マップの pathKey と同じ流儀で時間軸を除いた dim 値の数)。
// _globalMaxMap の `${metricKey}:${depth}` キー解決に使う。
export function rowDepthForSparkline(rowVals: any[]): number {
  if (_seriesTimeIdxInCurrentDims < 0) return rowVals.length;
  return rowVals.filter((_: any, i: number) => i !== _seriesTimeIdxInCurrentDims).length;
}

// 各 path 深さに対して時系列を組み立てる (親集計行でもスパークラインを表示できるように)。
// バケット単位の agg は基礎 metric を合算して派生を再計算 (sumAggs と同じ流儀)。
function sumBucketAggs(aggs: any[]): Record<string, number> {
...
export function prepareSparklineSeries(rows: any[], currentDims: string[]) {
  ...
  _pathDepthMap.clear();
  const temp = new Map<string, Map<any, any[]>>();
  for (const g of bucketGroups) {
    const fullVals = g.vals.filter((_: any, i: number) => i !== timeIdxInBucket);
    ...
    for (let k = 0; k <= fullVals.length; k++) {
      ...
      if (!timeMap) { timeMap = new Map<any, any[]>(); temp.set(pathKey, timeMap); }
