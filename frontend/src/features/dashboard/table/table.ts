...
import { escapeHtml, escapeHtmlNl } from '@shared/utils/utils.ts';
import { dimLabel, dimSort } from '@aggregate/dimensions.ts';
import { aggregate } from '@aggregate/aggregate.ts';
import { openTableSettings, buildHeaderCellStyle, buildTableStyle, subtotalAt } from './tableSettings.ts';
import { emit } from '@app/events.ts';
import { sumAggs, evalDerivedWithContext } from './aggUtils.ts';
import { passesFilter, makeBucketComparator, sortListFrom, customSortIndex } from './query.ts';
import { metricCellHtml, buildMetricCells, isSafeLink, isSafeImageSrc, dimCellHtml } from './cells.ts';
...
export function setTableState(state: any = {}) {
...
function collectGroupKeys(groups: any[], totalDimCount: number): Set<string> {
  ...
  function walk(bucket: any[], dimIndex: number, parentPath: any[]) {
    if (dimIndex >= totalDimCount - 1) return; // 最終 dim は leaf。toggle 無し
    const byVal = new Map<any, any[]>();
...
function buildFromGroups(groups: any[], dims: string[], metrics: any[], totalDimCount: number, totalAgg: any = null): string {
...
function takePending(cells: string[], pending: { index: number; html: string }[]) {
...
function buildLevel(groups: any[], dims: string[], dimIndex: number, totalDimCount: number, metrics: any[], parentPath: any[], parentAgg: any = null, totalAgg: any = null, pending: { index: number; html: string }[] = []): string {
  ...
  for (const val of sortedKeys) {
    ...
    if (isLastDim) {
      ...
      const myAgg = sumAggs(bucket.map((g: any) => g.agg), parentAgg, totalAgg);
...
function buildColumnTree(groups: any[], dims: string[], dimIndex: number, parentPath: any[], parentAgg: any, totalAgg: any, state: { truncated: boolean; count: number }, showSub: (d: number) => boolean): any[] {
  const out: any[] = [];
  const isLastDim = dimIndex === dims.length - 1;
  const buckets = new Map<any, any[]>();
  for (let i = 0; i < groups.length; i++) {
  ...
  for (const val of sortedKeys) {
    ...
    if (isLastDim) {
      ...
      if (!levelKeys[dimIndex]) levelKeys[dimIndex] = new Set();
      levelKeys[dimIndex].add(groupKey);
      const node: any = { isParent: true, hasOwnCol: false, label: val, depth: dimIndex, agg: myAgg, groupVals: path, groupKey, collapsed: isCollapsed, children: [] };
...
function flattenColumns(nodes: any[], out: any[] = []): any[] {
...
function nodeSpan(n: any): number {
...
function applyTransposeHeadOffsets() {
  ...
  table.querySelectorAll<HTMLElement>('thead tr').forEach(tr => {
    tr.querySelectorAll<HTMLElement>('th').forEach(th => { th.style.top = top + 'px'; });
    top += tr.offsetHeight;
  });
}
...
function setTransposeNotice(msg: string) {
...
function tposeHeaderInner(dimKey: string, value: any): string {
...
function buildTransposed(groups: any[], dims: string[], metrics: any[], totalAgg: any) {
  for (let i = 0; i < groups.length; i++) {
    if (!groups[i].agg) groups[i].agg = aggregate(groups[i].rows);
  }
  ...
  const headRows: string[][] = Array.from({ length: headRowCount }, () => []);
  ...
  const headCell = (n: any, depth: number, colspan: number, rowspan: number, label: any, isSub: boolean) => {
  ...
  const bodyRows = metrics.map((m: any) => {
...
function renderToolbar(dims: string[]) {
  ...
  if (dims.length >= 2) {
    ...
    levels.forEach((dimKey: string, i: number) => {
...
function applyFrozenColumns() {
  ...
  table.querySelectorAll('tbody tr, thead .total-row').forEach(tr => {
    ...
    for (let i = 0; i < frozen && i < tds.length; i++) {
      ...
      (tds[i] as HTMLElement).style.left = lefts[i] + 'px';
    }
  });
}
...
export function renderTable(groups: any[]) {
  const metrics = S.SELECTED_METRICS.map((k: string) => S.METRIC_DEFS.find((m: any) => m.key === k)).filter(Boolean);
  const dims = S.SELECTED_DIMS;
  // フィルタ適用 (groups[] = leaf 相当を絞る → 親集計も自動的にフィルタ後の値に)
  const filters = S.TABLE_CONFIG?.filters;
  if (filters && Object.keys(filters).length) {
    groups = groups.filter((g: any) => passesFilter(g, dims, filters));
  }
  const cols = [
    ...dims.map((k: string) => ({key: 'dim:' + k, label: dimLabel(k), isDim: true, defW: 130})),
    ...metrics.map((m: any) => ({key: 'met:' + m.key, label: m.label, isDim: false, defW: 110})),
  ];
  ...
  let totalAgg: any = null;
  if (groups.length) {
    const baseTotal = sumAggs(groups.map((g: any) => g.agg || aggregate(g.rows)));
  ...
  if (dims.length >= 2) {
    ...
    if (sortList.length) {
      const compareOne = (s: any, a: any, b: any) => {
        if (!s || !s.col) return 0;
      ...
      sortedGroups = [...groups].sort((a: any, b: any) => {
    ...
    bodyRows = sortedGroups.map((g: any) => {
      ...
      const dimCells = g.vals.map((v: any, i: number) => dimCellHtml(dims[i], v, '', null, i, depthOpts)).join('');
  ...
  if (S.TABLE_CONFIG?.showTotal && groups.length && totalAgg) {
    ...
    const totOpts = { skipColStyle: !!S.TABLE_CONFIG?.table?.totalPriority };
    const dimCells = dims.map((dk: string, i: number) => dimCellHtml(dk, '', 'total-label', i === 0 ? '総計' : '', i, totOpts)).join('');
...
document.getElementById('data-table')!.addEventListener('click', e => {
  const btn = (e.target as HTMLElement).closest('.pivot-toggle') as HTMLElement | null;
  if (!btn) return;
...
document.getElementById('table-toolbar')!.addEventListener('click', e => {
  ...
  if (action === 'expand-all') {
    ...
    document.querySelectorAll(`.pivot-toggle[data-pivot-level="${level}"]`).forEach(t => {
      collapsed.delete((t as HTMLElement).dataset.pivotKey!);
    });
  } else if (action === 'collapse-level' && level != null) {
    document.querySelectorAll(`.pivot-toggle[data-pivot-level="${level}"]`).forEach(t => {
      collapsed.add((t as HTMLElement).dataset.pivotKey!);
    });
...
let _filtersBarOriginalParent: (Node & ParentNode) | null = null;
let _filtersBarOriginalNext: Node | null = null;
...
function handleFreezeInput(input: HTMLInputElement) {
  ...
  if (v !== shown) {
    ...
    input.value = shown as any;
  }
}
document.getElementById('table-toolbar')!.addEventListener('keydown', e => {
  if (e.key === 'Enter' && (e.target as HTMLElement).dataset.toolbarAction === 'freeze-input') {
    e.preventDefault();
    handleFreezeInput(e.target as HTMLInputElement);
    (e.target as HTMLInputElement).blur();
  }
});
document.getElementById('table-toolbar')!.addEventListener('focusout', e => {
  if ((e.target as HTMLElement).dataset?.toolbarAction === 'freeze-input') {
    handleFreezeInput(e.target as HTMLInputElement);
  }
});
