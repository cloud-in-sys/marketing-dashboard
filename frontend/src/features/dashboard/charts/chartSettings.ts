...
import { getComboLines } from './chart.ts';
import { metricUsesHierarchy } from '@aggregate/aggregate.ts';

// ===== グラフ設定サイドパネル =====
export function openChartSettings(chartId: number) {
  ...
  body.innerHTML = `
    ...
    <div class="chart-settings-field">
      ...
      <div class="combo-lines">
        ${getComboLines(c).map((l: any, idx: number) => `
