...
import type { ChartConfig } from '@app/models.ts';
...
import { renderChartSettingsPanel, closeChartSettings } from './chartSettings.ts';
import { isAnyColorPickerOpen } from '@shared/ui/colorPicker.ts';
...
export function formatMetricValue(mdef: any, v: number): string {
...
export function smoothPath(pts: { cx: number; cy: number }[]): string {
...
function buildPieSVG(chart: ChartConfig, rows: any[], W: number, H: number): string {
  ...
  let data = groups.map((g: any) => ({x: g.vals[0], y: (g.agg || aggregate(g.rows))[chart.metric] || 0}))
    .filter((d: any) => d.y > 0)
  ...
  if (data.length > 12) {
    ...
    const otherY = data.slice(11).reduce((s: number, d: any) => s + d.y, 0);
    data = [...top, { x: 'その他', y: otherY }];
  }

  const total = data.reduce((s: number, d: any) => s + d.y, 0) || 1;
  ...
  S.CHART_POINTS.set(chart.id, {
    W, H, PT: 0, ih: H,
    points: slices.map((sl: any, i: number) => {
      const midAngle = -Math.PI / 2 + data.slice(0, i).reduce((s: number, d: any) => s + (d.y / total) * Math.PI * 2, 0) + (data[i].y / total) * Math.PI;
...
function buildStackedSVG(chart: ChartConfig, rows: any[], W: number, H: number): string {
  const mdef = S.METRIC_DEFS.find((m) => m.key === chart.metric);
  const xDim = chart.bucket && chart.bucket !== 'auto' ? chart.bucket : S.SELECTED_DIMS[0];
  ...
  const xOrder: any[] = [];
  const xSeen = new Set<any>();
  const stackOrder: any[] = [];
  const stackSeen = new Set<any>();
  const cell = new Map<string, number>(); // `${x}${s}` -> y
  ...
  // 合計の最大値(スケール用)
  const totals = xOrder.map(x => stackOrder.reduce((s: number, sv: any) => s + (cell.get(`${x}${sv}`) || 0), 0));
  ...
  xOrder.forEach((xv, i) => {
    ...
    stackOrder.forEach((sv, si: number) => {
  ...
  // 凡例 (右上)
  stackOrder.slice(0, 6).forEach((sv, si: number) => {
    const color = STACK_PALETTE[si % STACK_PALETTE.length];
    const lx = W - PR - 120;
    const ly = 10 + si * 14;
...
export function getComboLines(chart: ChartConfig): any[] {
...
function buildComboSVG(chart: ChartConfig, rows: any[], W: number, H: number): string {
  ...
  const lineDefs = getComboLines(chart)
    .map((l: any) => ({ ...l, mdef: S.METRIC_DEFS.find((m) => m.key === l.metric) }))
    .filter((l: any) => l.mdef);
  ...
  const data = groups.map((g: any) => {
    ...
    const row: Record<string, any> = { x: g.vals[0], y1: a[chart.metric] || 0 };
    lineDefs.forEach((l: any, idx: number) => { row[`y${idx + 2}`] = a[l.metric] || 0; });
    return row;
  });
  if (!data.length) return '<div style="padding:24px;text-align:center;color:#64748b;font-size:12px">データなし</div>';
  ...
  const iw = W - PL - PR, ih = H - PT - PB;
  const max1 = Math.max(...data.map((d: any) => d.y1), 0) || 1;
  // 右軸は複数線の最大値を共有
  const maxR = Math.max(
    ...data.flatMap((d: any) => lineDefs.map((_: any, idx: number) => d[`y${idx + 2}`])),
    0
  ) || 1;
  ...
  S.CHART_POINTS.set(chart.id, {
    ...
    points: data.map((d: any, i: number) => {
      ...
      lineDefs.forEach((l: any, idx: number) => { parts.push(`${escapeHtml(l.mdef.label)}: ${formatMetricValue(l.mdef, d[`y${idx + 2}`])}`); });
  ...
  // 折れ線群
  const linePtsByIdx = lineDefs.map((l: any, idx: number) =>
  ...
  lineDefs.forEach((l: any, idx: number) => {
  ...
  // 凡例
  const legendItems = [{ color: color1, label: m1.label }, ...lineDefs.map((l: any) => ({ color: l.color, label: l.mdef.label }))];
  legendItems.forEach((li: any, i: number) => {
    const lx = PL + i * 110;
...
// ===== Chart rendering =====
export function buildChartSVG(chart: ChartConfig, rows: any[], W: number, H: number): string {
  ...
  if (!xDim) return '<div style="padding:24px;text-align:center;color:#64748b;font-size:12px">ディメンションを選択してください</div>';
  const groups = groupRows(rows, [xDim]);
  const data: Record<string, any>[] = groups.map((g: any) => ({x: g.vals[0], y: (g.agg || aggregate(g.rows))[chart.metric] || 0}));
  if (!data.length) return '<div style="padding:24px;text-align:center;color:#64748b;font-size:12px">データなし</div>';
  ...
  if (extraLines.length) {
    data.forEach((d: any, i: number) => {
      ...
      extraLines.forEach((l: any, idx: number) => { d[`y${idx + 2}`] = a[l.metric] || 0; });
    });
  }
  ...
  const maxY = extraLines.length
    ? Math.max(...data.map((d: any) => d.y), ...data.flatMap((d: any) => extraLines.map((_: any, idx: number) => d[`y${idx + 2}`])), 0) || 1
    : Math.max(...data.map((d: any) => d.y), 0) || 1;
  const step = iw / data.length;
  const barW = Math.max(0, Math.min(step * 0.7, 36));
  ...
  const pts = data.map((d: any, i: number) => ({cx: PL + step * i + step / 2, cy: PT + ih - ih * (d.y / maxY), d}));
  S.CHART_POINTS.set(chart.id, {
    W, H, PT, ih,
    points: pts.map((p: any) => {
      if (extraLines.length) {
        const labels = [`${mdef.label}: ${yFmt(p.d.y)}`];
        extraLines.forEach((l: any, idx: number) => {
  ...
  if (chart.type === 'bar') {
    ...
    if (chart.smoothLine) {
      ...
      const linePts = pts.map((p: any) => `${p.cx},${p.cy}`).join(' ');
    ...
    extraLines.forEach((l: any, idx: number) => {
      const linePts = data.map((d: any, i: number) => ({
        cx: PL + step * i + step / 2,
      ...
      if (chart.smoothLine) {
        ...
        s += `<polyline points="${linePts.map((p: any) => `${p.cx},${p.cy}`).join(' ')}" fill="none" stroke="${l.color}" stroke-width="${lw}" stroke-linejoin="round" stroke-linecap="round"/>`;
      }
      if (chart.showDots !== false) {
        const r = chart.dotSize ?? 3.5;
        linePts.forEach((p: any) => {
  ...
  // 凡例 (追加の折れ線がある場合のみ)
  if (extraLines.length) {
    const legendItems = [{ color, label: mdef.label }, ...extraLines.map((l: any) => ({ color: l.color, label: l.mdef.label }))];
    legendItems.forEach((li: any, i: number) => {
      const lx = PL + i * 110;
      s += `<rect x="${lx}" y="2" width="10" height="10" fill="${li.color}" rx="2"/>`;
      s += `<text x="${lx + 14}" y="11" font-size="10" fill="#334155">${escapeHtml(li.label.slice(0, 12))}</text>`;
...
export function renderChart(rows: any[]) {
  ...
  `).join('');
  S.CHARTS.forEach((c) => {
    const body = grid.querySelector(`[data-chart-body="${c.id}"]`) as HTMLElement | null;
    if (!body) return;
