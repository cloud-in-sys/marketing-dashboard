...
import { S, PALETTE, saveState } from '@app/state.ts';
import type { ChartConfig } from '@app/models.ts';
import { openChartSettings, closeChartSettings, renderChartSettingsPanel } from './chartSettings.ts';
...
document.getElementById('charts-grid')!.addEventListener('click', e => {
  const card = (e.target as HTMLElement).closest('.chart-card') as HTMLElement | null;
  if (!card) return;
  if (!canEditCurrentTab()) return;
  const id = +card.dataset.id!;
  const removeBtn = (e.target as HTMLElement).closest('[data-role="remove"]');
  if (removeBtn) {
  ...
  const settingsBtn = (e.target as HTMLElement).closest('[data-role="settings"]');
  if (settingsBtn) {
...
function ensureLines(chart: ChartConfig): any[] {
...
function onPanelClick(e: Event) {
  const btn = (e.target as HTMLElement).closest('[data-panel-role]') as HTMLElement | null;
  if (!btn) return;
  ...
  if (role === 'line-add') {
    const lines = ensureLines(chart);
    const used = new Set(lines.map((l: any) => l.metric));
