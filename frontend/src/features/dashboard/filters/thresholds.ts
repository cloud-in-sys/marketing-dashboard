...
function toDisplayThreshold(v: any, fmt?: string): string | number {
  if (v == null) return '';
  return fmt === 'pct' ? v * 100 : v;
}
function fromDisplayThreshold(v: any, fmtType?: string): number | null {
...
function renderThresholds() {
  ...
  if (!S.THRESHOLD_METRICS.length) {
    ...
    el.innerHTML = S.THRESHOLD_METRICS.map((k: string) => {
      ...
      const opSelect = (role: string, val: string) => `<select class="threshold-op" data-role="${role}Op">
  ...
  menu.innerHTML = avail.length
    ? avail.map((m: any) => `<button type="button" class="add-menu-item" data-add="${m.key}">${m.label}</button>`).join('')
...
document.getElementById('threshold-list')!.addEventListener('input', e => {
  const row = (e.target as HTMLElement).closest('.threshold-row') as HTMLElement | null;
  if (!row) return;
  ...
  if (role.endsWith('Op')) {
    S.THRESHOLDS[key][role] = (e.target as HTMLInputElement).value;
  } else {
    const mdef = S.METRIC_DEFS.find((m) => m.key === key);
    if (!mdef) return;
    const raw = fromDisplayThreshold((e.target as HTMLInputElement).value, mdef.fmt);
...
document.getElementById('threshold-list')!.addEventListener('click', e => {
  ...
  S.THRESHOLD_METRICS = S.THRESHOLD_METRICS.filter((k: string) => k !== key);
...
document.addEventListener('click', e => {
  if (!(e.target as HTMLElement).closest('#threshold-add-btn') && !(e.target as HTMLElement).closest('#threshold-add-menu')) {
    document.getElementById('threshold-add-menu')!.classList.add('hidden');
  }
});
