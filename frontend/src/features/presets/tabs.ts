import { S, getPresets, syncCurrentTabState, setUserCurrentView } from '@app/state.ts';
import type { CustomTab } from '@app/models.ts';
import { escapeHtml, hexToSoft } from '@shared/utils/utils.ts';
import { loadTabState, exitPresetEdit, renderTabPresetSelect } from './presets.ts';
...
function readCollapsedGroups(): Set<string> {
  try { return new Set<string>(JSON.parse(localStorage.getItem(COLLAPSED_GROUPS_KEY) || '[]')); }
...
export function toggleCustomTabGroup(name: string) {
...
function renderCustomTabItem(t: CustomTab): string {
...
export function renderCustomTabs() {
  ...
  const ungrouped: any[] = [];
  const groups = new Map<string, any[]>(); // name \u2192 tabs[]
  for (const t of S.CUSTOM_TABS) {
    const g = t.group;
...
export function renderViewNav() {
  ...
  el.innerHTML = S.VIEW_ORDER.map(k => {
    ...
    const preset = presets.find((p: any) => p.builtin && p.name === label);
...
export async function applyView(viewKey: string) {
...
export function highlightActiveView() {
  document.querySelectorAll<HTMLElement>('#view-nav .nav-item').forEach(b => {
