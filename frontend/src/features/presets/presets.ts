...
import { getTableState, setTableState } from '../dashboard/table/table.ts';
...
interface BuiltinPresetDef {
  charts: { metric: string; type: string; size: string; color: string }[];
  metrics: string[];
  thresholdMetrics?: string[];
  thresholds?: Record<string, any>;
}

export const BUILTIN_PRESET_DEFS: Record<string, BuiltinPresetDef> = {
  summary_daily: {
    charts: [
...
export async function seedDefaultPresets() {
  ...
  for (const [k, v] of Object.entries(S.VIEWS)) {
    const initDef = (DEFAULT_VIEWS_INIT as Record<string, any>)[k];
    const presetDef = initDef ? (BUILTIN_PRESET_DEFS[k] || null) : null;
    // thresholds / thresholdMetrics は optional なので、フォールバック側に無くてよい
    const def: BuiltinPresetDef = presetDef || {
      charts: [{metric: 'ad_cost', type: 'bar', size: 'main', color: '#2563eb'}],
      metrics: S.METRIC_DEFS.map((m: any) => m.key),
    };
    const preset = {
      name: v.presetName || v.label,
      builtin: true,
      seedVersion: BUILTIN_SEED_VERSION,
      charts: def.charts.map((c: any, i: number) => ({id: i + 1, metric: c.metric, type: c.type, size: c.size, color: c.color, name: '', bucket: 'auto'})),
      dims: [...v.dims],
      metrics: [...def.metrics],
...
function deserializeFilterValues(values: any): Record<string, any> {
...
export function loadPresetIntoGlobals(p: any) {
  // 全フィールド保持(lines, smoothLine, dotSize, lineWidth, showDataLabels 等)
  // p.charts が空/未定義でも S.CHARTS を必ずリセット (= 削除タブの古いチャートが残らない)
  S.CHARTS = Array.isArray(p.charts) ? p.charts.map((c: any) => ({...c, color: c.color || '#2563eb', name: c.name || '', bucket: c.bucket || 'auto'})) : [];
  S.CHART_ID_SEQ = S.CHARTS.length ? Math.max(0, ...S.CHARTS.map((c: any) => c.id)) + 1 : 1;
...
export function applyPresetFilters(p: any) {
...
export async function savePresetPrompt() {
  ...
  const userCount = list.filter((p: any) => !p.builtin).length;
  const entry = {
    name,
    color: PRESET_COLORS[userCount % PRESET_COLORS.length],
    charts: S.CHARTS.map((c: any) => ({...c})),
...
export async function duplicatePreset(i: number) {
  ...
  let name = base;
  let n = 2;
  while (list.some((p: any) => p.name === name)) name = `${base} ${n++}`;
  ...
  if (list.some((p: any) => p.name === input)) {
  ...
  try {
    ...
    if (srcId && created?.id) {
      ...
      const srcIdx = cache.findIndex((p: any) => p.id === srcId);
      const newIdx = cache.findIndex((p: any) => p.id === created.id);
      if (srcIdx >= 0 && newIdx >= 0 && newIdx !== srcIdx + 1) {
        const orderIds = cache.map((p: any) => p.id);
...
export async function renamePreset(i: number) {
  const list = getPresets();
  const target = list[i];
  ...
  if (list.some((p: any) => p.id !== target.id && p.name === newName)) {
...
export async function deletePreset(i: number) {
  const list = getPresets();
  const target = list[i];
...
let _presetEditSnapshot: string | null = null;

// syncPresetEdit と同じフィールドをシリアライズ (save 内容と一致させる)。
function computePresetEditSnapshot(): string | null {
  if (S.PRESET_EDIT_IDX == null) return null;
  const picker = document.getElementById('preset-color-picker') as any;
...
export function syncPresetEdit() {
  ...
  const updated = {
    ...
    filterValues: serializeFilterValues(S.FILTER_VALUES),
    filterConditions: JSON.parse(JSON.stringify(S.FILTER_CONDITIONS || {})),
    color: (document.getElementById('preset-color-picker') as any).value || p.color,
  };
...
export function enterPresetEdit(idx: number) {
  ...
  (document.getElementById('preset-color-picker') as any).value = p.color || (p.builtin ? '#2563eb' : '#64748b');
  emit('renderChips');
  emit('renderThresholds');
  renderPresets();
  emit('render');
  const viewEl = document.querySelector('.view') as HTMLElement | null;
...
// 保存値が無いタブは空フィルタにリセット。
function restoreFilterStateForTab(viewKey: string | null) {
  ...
  for (const [k, v] of Object.entries<any>(fs.filterValues)) {
...
// viewKey が null (タブが 1 つも無い状態) でも安全: VIEWS/CUSTOM_TABS の索引は
// undefined になり、両クラスとも外れる = 既定の見た目になる。
export function syncTabBodyClasses(viewKey: string | null) {
...
// viewKey が null でも安全: VIEWS にも TAB_STATES にも当たらず、既定値 (空タブ) で
// 初期化される。タブを全部削除した直後の reloadFullUI がこの経路を通る。
export function loadTabState(viewKey: string | null) {
  ...
  if (S.VIEWS[viewKey!]) {
    ...
    const p = getPresets().find((x: any) => x.name === presetName);
    if (p) {
      loadPresetIntoGlobals(p);
      applyPresetFilters(p);
    } else {
...
export function renderTabPresetSelect() {
  // null 索引は undefined になるだけ (!! で false)。実行時の挙動は変えていない。
  document.body.classList.toggle('readonly-tab', !!S.VIEWS[S.CURRENT_VIEW!]);
  const sel = document.getElementById('tab-preset') as HTMLSelectElement | null;
  if (!sel) return;
  const list = getPresets();
  sel.innerHTML = '<option value="">\u2014 \u9078\u629e \u2014</option>' + list.map((p: any, i: number) => `<option value="${i}">${p.builtin ? '[\u6a19\u6e96] ' : ''}${escapeHtml(p.name)}</option>`).join('');
  const tabPreset = S.CUSTOM_TABS.find((t) => t.key === S.CURRENT_VIEW)?.presetName;
  const idx = tabPreset ? list.findIndex((p: any) => p.name === tabPreset) : -1;
  sel.value = idx >= 0 ? String(idx) : '';
}

export async function createBuiltinPresetFor(baseName: string): Promise<string | null> {
  const list = getPresets();
  let name = baseName;
  let n = 2;
  while (list.some((p: any) => p.name === name)) { name = `${baseName} (${n++})`; }
  const preset = {
    name,
    builtin: true,
    seedVersion: BUILTIN_SEED_VERSION,
...
let _exitSettingsMode: (() => void) | null = null;
export function setExitSettingsMode(fn: () => void) { _exitSettingsMode = fn; }
