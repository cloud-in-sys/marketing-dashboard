...
import { SIDEBAR_KEY, PANELS_KEY, GROUPS_KEY } from '@app/state.ts';
...
// ===== SIDEBAR HELPERS =====
function getCollapsedPanels(): Set<string> {
  try { return new Set<string>(JSON.parse(localStorage.getItem(PANELS_KEY) || '[]')); }
  catch (e) { return new Set<string>(); }
}
function setCollapsedPanels(set: Set<string>) {
...
function initPanelCollapse() {
  ...
  document.querySelectorAll('.panel.collapsible').forEach(p => {
    if (collapsed.has((p as HTMLElement).dataset.panel!)) p.classList.add('collapsed');
  });
}
...
function setSidebarGroupState(s: any) {
...
function applySidebarGroupState() {
  ...
  document.querySelectorAll('.sidebar-group').forEach(g => {
    g.classList.toggle('collapsed', !!s.collapsed?.[(g as HTMLElement).dataset.group!]);
  });
}
...
document.querySelectorAll('.sidebar-group .group-title').forEach(title => {
  title.addEventListener('click', () => {
    const group = title.closest('.sidebar-group') as HTMLElement;
