...
import { parseBaseFormula, isPureBaseFormula } from '@aggregate/aggregate.ts';
import { hasPerm } from '@app/auth.ts';
import { renderViewNav } from '../../presets/tabs.ts';
...
export function renderMetricsDoc() {
    ...
    return `<div class="metrics-doc-row" data-def-idx="${i}" data-drag-key="${i}" draggable="true">
      <div class="metrics-doc-row-head">
        ...
        <div class="field-col"><label class="field-label">書式</label><select class="metric-fmt-select" data-def-fmt draggable="false">
          ${fmtOptions.map((o: any) => `<option value="${o.v}"${m.fmt===o.v?' selected':''}>${o.l}</option>`).join('')}
        </select></div>
  ...
  const baseRows = defs.map((m: any, i: number) => ({m, i})).filter((x: any) => x.m.type === 'base');
  const derivedRows = defs.map((m: any, i: number) => ({m, i})).filter((x: any) => x.m.type === 'derived');
  // 既存の基礎メトリクスにルール違反 (単一集計でない式) があれば警告を出す。
  // ピボット親行で値が壊れるので「派生」に変える必要がある旨を案内。
  const violators = baseRows
    .map((x: any) => ({ m: x.m, formula: (baseSrc[x.m.key] || DEFAULT_BASE_FORMULAS[x.m.key] || '') }))
    .filter((x: any) => x.formula && !isPureBaseFormula(x.formula));
  const warningHtml = violators.length
    ? `<div class="metrics-doc-warning">
        ...
        <ul>${violators.map((v: any) => `<li><strong>${escapeHtml(v.m.label || v.m.key)}</strong>: <code>${escapeHtml(v.formula)}</code></li>`).join('')}</ul>
      </div>`
    : '';
  el.innerHTML = `
    ...
    <div class="metrics-doc-box">
      ...
      ${baseRows.map((x: any) => renderRow(x.m, x.i)).join('') || '<div class="preset-empty">基礎メトリクスがありません</div>'}
    ...
    <div class="metrics-doc-box">
      ...
      ${derivedRows.map((x: any) => renderRow(x.m, x.i)).join('') || '<div class="preset-empty">派生メトリクスがありません</div>'}
  ...
  el.querySelectorAll<HTMLElement>('.metrics-doc-row[data-def-idx]').forEach(row => {
    const idx = +row.dataset.defIdx!;
    const def = defs[idx];
    const ta = row.querySelector('[data-def-label]') as HTMLTextAreaElement | null;
...
function autosizeLabel(el: HTMLTextAreaElement) {
...
export function setupMetricsEvents() {
  ...
  document.getElementById('metrics-doc')!.addEventListener('input', e => {
    if (!hasPerm("editMetrics")) return;
    const row = (e.target as HTMLElement).closest('[data-def-idx]') as HTMLElement | null;
    if (!row) return;
    const idx = +row.dataset.defIdx!;
    const defs = S.METRIC_DEFS_DRAFT || S.METRIC_DEFS;
    const def = defs[idx];
    if (!def) return;
    if ((e.target as HTMLElement).matches('[data-def-label]')) {
      // 5 行上限: 超えたら切り詰めてカーソルを末尾に
      const lines = (e.target as HTMLTextAreaElement).value.split('\n');
      if (lines.length > 5) (e.target as HTMLTextAreaElement).value = lines.slice(0, 5).join('\n');
      def.label = (e.target as HTMLTextAreaElement).value;
      autosizeLabel(e.target as HTMLTextAreaElement);
    }
    else if ((e.target as HTMLElement).matches('[data-def-key]')) {
    ...
    // option は fmtOptions から描画しているので必ず MetricFormat のどれか
    else if ((e.target as HTMLElement).matches('[data-def-fmt]')) def.fmt = (e.target as HTMLSelectElement).value as MetricDefinition['fmt'];
    else if ((e.target as HTMLElement).matches('[data-def-formula]')) {
      const k = def.key;
  ...
  document.getElementById('metrics-doc')!.addEventListener('click', async e => {
    if (!hasPerm("editMetrics")) return;
    const rm = (e.target as HTMLElement).closest('[data-def-remove]') as HTMLElement | null;
    if (rm) {
    ...
    const add = (e.target as HTMLElement).closest('[data-add-type]') as HTMLElement | null;
    if (add) {
      // ボタンは data-add-type="base" / "derived" の 2 種類だけ描画している
      const type = add.dataset.addType as MetricDefinition['type'];
      const defs = S.METRIC_DEFS_DRAFT || S.METRIC_DEFS;
      let base = 'metric';
      let n = 1;
      let newKey = base + n;
      while (defs.some((d: any) => d.key === newKey)) { n++; newKey = base + n; }
...
CTR / CPC / CPM 等の割り算は派生型にして、基礎メトリクスを参照する式 (例: clicks / impression) にしてください。多重ディメンションのピボット親行で値が壊れるのを防ぐためです。`,
  ...
  document.getElementById('metrics-help-btn')!.addEventListener('click', () => {
    const help = document.getElementById('metrics-syntax-help') as HTMLDetailsElement | null;
    if (!help) return;
    help.open = true;
