import { S, saveDimensions } from '@app/state.ts';
...
export function renderDimsDoc() {
          ...
          <div class="dim-row-grid">
            <div class="dim-field">
              ...
              <select class="dim-type-select" data-dim-type draggable="false">
                ${typeOpts.map((o: any) => `<option value="${o.v}"${d.type===o.v?' selected':''}>${o.l}</option>`).join('')}
              </select>
            </div>
            ...
            <div class="dim-field">
              ...
              <select class="dim-type-select" data-dim-week-start draggable="false">
                ${weekStartOpts.map((o: any) => `<option value="${o.v}"${ws===o.v?' selected':''}>${o.l}</option>`).join('')}
              </select>
            </div>` : ''}
          </div>
        </div>`;
  ...
  el.querySelectorAll<HTMLElement>('.dim-row[data-dim-idx]').forEach(row => {
    const idx = +row.dataset.dimIdx!;
    const def = defs[idx];
    const ta = row.querySelector('[data-dim-label]') as HTMLTextAreaElement | null;
...
export function setupDimensionsEvents() {
  ...
  document.getElementById('dims-doc')!.addEventListener('input', e => {
    if (!hasPerm('editDimensions')) return;
    const row = (e.target as HTMLElement).closest('[data-dim-idx]') as HTMLElement | null;
    if (!row) return;
    const idx = +row.dataset.dimIdx!;
    const defs = S.DIMENSIONS_DRAFT || [];
    const def = defs[idx];
    if (!def) return;
    if ((e.target as HTMLElement).matches('[data-dim-label]')) {
    ...
    else if ((e.target as HTMLElement).matches('[data-dim-field]')) def.field = (e.target as HTMLInputElement).value;
    else if ((e.target as HTMLElement).matches('[data-dim-expr]')) def.expression = (e.target as HTMLInputElement).value;
    else if ((e.target as HTMLElement).matches('[data-dim-image-height]')) {
      const v = (e.target as HTMLInputElement).value;
      def.imageHeight = v === '' ? null : Math.max(8, Math.min(400, Number(v) || 40));
    } else if ((e.target as HTMLElement).matches('[data-dim-image-width]')) {
      const v = (e.target as HTMLInputElement).value;
  ...
  document.getElementById('dims-doc')!.addEventListener('change', e => {
    ...
    const def = defs[idx];
    if (!def) return;
    if ((e.target as HTMLElement).matches('[data-dim-type]')) {
      // option は typeOpts から描画しているので必ず DimensionKind のどれか
      def.type = (e.target as HTMLSelectElement).value as DimensionDefinition['type'];
      ...
      markDimsDirty();
      renderDimsDoc();
    } else if ((e.target as HTMLElement).matches('[data-dim-week-start]')) {
  ...
  document.getElementById('dims-doc')!.addEventListener('click', async e => {
    if (!hasPerm('editDimensions')) return;
    if ((e.target as HTMLElement).closest('#dims-add-btn')) {
      const defs = S.DIMENSIONS_DRAFT || [];
      let n = 1;
      let key = 'dim' + n;
      while (defs.some((d: any) => d.key === key)) { n++; key = 'dim' + n; }
      defs.push({key, label: '新規ディメンション', field: '', type: 'value'});
      markDimsDirty();
      renderDimsDoc();
      return;
    }
    const rm = (e.target as HTMLElement).closest('[data-dim-remove]') as HTMLElement | null;
    if (rm) {
  ...
  document.getElementById('dims-save-btn')!.addEventListener('click', async () => {
    ...
    if (keys.some((k: any) => !k)) { await showModal({title: '保存できません', body: '空のキーがあります', okText: 'OK', cancelText: ''}); return; }
    ...
    try {
      ...
      S.SELECTED_DIMS = S.SELECTED_DIMS.filter((k: string) => validKeys.has(k));
      emit('renderChips');
      try {
