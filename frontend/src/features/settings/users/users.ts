import { S, PERM_GROUPS, PERM_DEFS, ADMIN_PERMS, VIEWER_PERMS, SETTINGS_PERM_KEYS } from '@app/state.ts';
import type { PermissionKey } from '@app/models.ts';
import { api } from '@api/index.ts';
import { escapeHtml } from '@shared/utils/utils.ts';
import { showModal } from '@shared/ui/modal.ts';
import { getCurrentUser, renderCurrentUserLabel, applyPermissionUI } from '@app/auth.ts';
...
// ===== USER ROLES =====
// 運用者 (operator) の権限セット = settings 群以外を全部持ち、settings 群は持たない。
// 区分は packages/shared が持つ (backend の isOperator / app/auth.js と同じ土台)。
const OPERATOR_PERMS: Record<string, boolean> = Object.fromEntries(
  PERM_DEFS.map(p => [p.key, !SETTINGS_PERM_KEYS.includes(p.key)])
);
...
type RoleKey = 'admin' | 'operator' | 'operatorPlus' | 'viewer' | 'viewerPlus' | 'custom';

function getUserRole(u: any): RoleKey {
  if (u.isAdmin) return 'admin';
  const perms = u.perms || {};
  const matches = (preset: Record<string, boolean>) => PERM_DEFS.every(p => !!perms[p.key] === !!preset[p.key]);
  // プリセットちょうどならそのロール名
...
function applyRole(u: any, role: string) {
  if (role === 'admin') {
    u.isAdmin = true;
    u.perms = {...ADMIN_PERMS};
...
// ----- USERS VIEW -----
const ROLE_LABEL: Record<RoleKey, string> = { admin: '管理者', operator: '運用者', operatorPlus: '運用者+', viewer: '一般', viewerPlus: '一般+', custom: 'カスタム' };
...
function renderUserListView(list: HTMLElement, src: any[]) {
  list.innerHTML = `<div class="user-list-compact">${src.map((u: any, i: number) => {
...
function renderUserDetail(list: HTMLElement, src: any[], i: number) {
  const u = src[i];
  const role = getUserRole(u);
  list.innerHTML = `
      ...
      <div class="user-field-grid">
        ...
        <div class="user-field">
          ...
          <select data-user-group>
            ...
            ${settingsState.groupsCache.map((g: any) => `<option value="${escapeHtml(g.id)}"${u.groupId === g.id ? ' selected' : ''}>${escapeHtml(g.name)}</option>`).join('')}
          </select>
        </div>
      </div>
      <div class="user-perms-section">
        <div class="user-perms-title">操作権限</div>
        ${PERM_GROUPS.map((g: any) => `
          <div class="user-perms-group">
            <div class="user-perms-group-label">${g.label}</div>
            <div class="user-perms">
              ${g.perms.filter((p: any) => !p.parent).map((p: any) => {
                const children = g.perms.filter((c: any) => c.parent === p.key);
                ...
                return `<div class="perm-with-children">${self}
                  ${parentOn ? `<div class="perm-children">${children.map((c: any) =>
...
async function removeUser(idx: number) {
  ...
  if (!ok) return;
  if (u.isAdmin && draft.filter((x: any) => x.isAdmin).length <= 1) {
...
export function setupUsersEvents() {
  ...
  document.getElementById('users-list')!.addEventListener('click', e => {
    const del = (e.target as HTMLElement).closest('[data-user-del]') as HTMLElement | null;
    if (del) { removeUser(+del.dataset.userDel!); return; }
    const back = (e.target as HTMLElement).closest('[data-user-back]');
    if (back) { settingsState.userDetailIdx = null; renderUsersModal(); return; }
    const open = (e.target as HTMLElement).closest('[data-user-open]') as HTMLElement | null;
  ...
  document.getElementById('users-list')!.addEventListener('input', e => {
    const row = (e.target as HTMLElement).closest('[data-user-idx]') as HTMLElement | null;
    if (!row) return;
    const idx = +row.dataset.userIdx!;
    const draft = S.USERS_DRAFT || S.USERS;
    const u = draft[idx];
    if (!u) return;
    if ((e.target as HTMLElement).matches('[data-user-name]')) u.name = (e.target as HTMLInputElement).value;
    else if ((e.target as HTMLElement).matches('[data-perm]')) {
      // data-perm は PERM_DEFS から描画しているので必ず実在キー
      const key = (e.target as HTMLElement).dataset.perm! as PermissionKey;
      u.perms![key] = (e.target as HTMLInputElement).checked;
    ...
    else if ((e.target as HTMLElement).matches('[data-user-group]')) u.groupId = (e.target as HTMLSelectElement).value || null;
    markUsersDirty();
  });
  document.getElementById('users-list')!.addEventListener('change', e => {
    ...
    const u = draft[idx];
    if (!u) return;
    if ((e.target as HTMLElement).matches('[data-user-role]')) {
      applyRole(u, (e.target as HTMLSelectElement).value);
      markUsersDirty();
      renderUsersModal();
    }
  });
  document.getElementById('users-save-btn')!.addEventListener('click', async () => {
    ...
    if (!S.USERS_DRAFT.some((u: any) => u.isAdmin)) {
    ...
    try {
      ...
      const originalMap: Record<string, any> = Object.fromEntries(S.USERS.map((u: any) => [u.uid, u]));
      ...
      for (const orig of S.USERS) {
        if (!S.USERS_DRAFT.some((u: any) => u.uid === orig.uid)) {
