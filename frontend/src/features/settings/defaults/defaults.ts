import { S, saveViews, saveViewOrder, getPresets, updatePresetOp, compileFilter } from '@app/state.ts';
...
import { renderPresets, renderTabPresetSelect, createBuiltinPresetFor } from '../../presets/presets.ts';
        ...
        <div class="metrics-doc-row defaults-row" data-view-idx="${i}">
          ...
          <div class="defaults-row-preset">
            ...
            <select class="defaults-preset-select" data-view-preset>
              ...
              ${presets.map((p: any) => `<option value="${escapeHtml(p.name)}"${v.presetName===p.name?' selected':''}>${p.builtin?'[標準] ':''}${escapeHtml(p.name)}</option>`).join('')}
            </select>
          </div>
        </div>
...
export function setupDefaultsEvents() {
  ...
  document.getElementById('defaults-doc')!.addEventListener('input', e => {
    ...
    const row = (e.target as HTMLElement).closest('[data-view-idx]') as HTMLElement | null;
    if (!row) return;
    const idx = +row.dataset.viewIdx!;
    const defs = S.VIEWS_DRAFT || [];
    const def = defs[idx];
    if (!def) return;
    if ((e.target as HTMLElement).matches('[data-view-label]')) def.label = (e.target as HTMLInputElement).value;
    markDefaultsDirty();
  });
  document.getElementById('defaults-doc')!.addEventListener('change', e => {
    ...
    const def = defs[idx];
    if (!def) return;
    if ((e.target as HTMLElement).matches('[data-view-preset]')) {
      def.presetName = (e.target as HTMLSelectElement).value;
      markDefaultsDirty();
    }
  });
  document.getElementById('defaults-doc')!.addEventListener('click', async e => {
    if (!hasPerm('editDefaults')) return;
    if ((e.target as HTMLElement).closest('#defaults-add-btn')) {
      const defs = S.VIEWS_DRAFT || [];
      const key = 'view_' + Date.now();
      const existingLabels = new Set(defs.map((v: any) => v.label));
      let label = '新規タブ';
      let n = 2;
    ...
    const rm = (e.target as HTMLElement).closest('[data-view-remove]') as HTMLElement | null;
    if (rm) {
  ...
  document.getElementById('defaults-save-btn')!.addEventListener('click', async () => {
    if (!hasPerm('editDefaults')) return;
    const defs = S.VIEWS_DRAFT || [];
    const keys = defs.map((v: any) => v.key);
    ...
    const oldLabels: Record<string, any> = {};
    const oldPresetNames: Record<string, any> = {};
    ...
    try {
      ...
      for (const v of defs) {
        if (!v.presetName) {
          ...
          if (oldLabel && oldLabel !== v.label && v.presetName === oldPresetName) {
            const preset = presetList.find((p: any) => p.name === oldPresetName && p.builtin);
            if (preset && preset.id) {
              try {
      ...
      S.VIEWS = next;
      saveViews();
      S.VIEW_ORDER = S.VIEW_ORDER.filter((k: string) => S.VIEWS[k]);
