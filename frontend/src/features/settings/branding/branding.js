...
import { applyBranding } from '../../sources/branding.js';
...
function renderImagePreview(url, kind) {
...
export function setupBrandingEvents() {
  ...
  form.addEventListener('input', e => {
    ...
    if (pickerHost) {
      ...
      const stops = [
        ...
        form.querySelector('[data-branding-stop="1"]').value,
        form.querySelector('[data-branding-stop="2"]').value,
      ];
      draft.headerGradient = gradientFromStops(stops);
      form.querySelector('.branding-gradient-preview').style.background = draft.headerGradient;
      markDirty();
  ...
  form.addEventListener('click', e => {
    const btn = e.target.closest('[data-branding-clear]');
    if (!btn) return;
    if (!hasPerm('manageBranding')) return;
