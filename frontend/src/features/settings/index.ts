...
import { hasPerm, getCurrentUser, logout } from '@app/auth.ts';
import { renderPresets, exitPresetEdit, isPresetEditDirty } from '../presets/presets.ts';
// 依存の無い小モジュール。sources.ts 本体を import すると循環になるが、これは安全。
import { getCurrentSourceSignal } from '../sources/sourceSignal.ts';
import { abortInFlightAggregate } from '@aggregate/aggregateBackend.ts';
import { emit } from '@app/events.ts';

import { settingsState } from './state.ts';
import { renderUsersModal, setupUsersEvents, clearUsersDirty } from './users/users.ts';
import { renderMetricsDoc, setupMetricsEvents, clearMetricsDirty } from './metrics/metrics.ts';
import { renderFiltersDoc, setupFilterDefsEvents, clearFiltersDirty } from './filters/filterDefs.ts';
import { renderDimsDoc, setupDimensionsEvents, clearDimsDirty } from './dimensions/dimensions.ts';
import { renderDefaultsDoc, setupDefaultsEvents, clearDefaultsDirty } from './defaults/defaults.ts';
import { loadGroupsAndRender, setupGroupsEvents } from './groups/groups.ts';
import { loadBrandingForEdit, setupBrandingEvents } from './branding/branding.ts';
...
function _doEnterSettingsMode(target: string) {
  ...
  if (target === 'users') {
    ...
    Promise.all([api.listUsers(), api.listGroups()]).then(([uRes, gRes]: [any, any]) => {
      ...
      if (isDirty) {
        S.USERS = serverUsers;
        const draftUids = new Set((S.USERS_DRAFT || []).map((u: any) => u.uid));
      ...
      renderUsersModal();
    }).catch((e: any) => console.warn('[users] load failed', e));
...
let _columnFetchInflight: Promise<any> | null = null;
async function fetchColumnInfoIfNeeded() {
  ...
  // source 補助 API 用の AbortSignal。source 本体 (sources.ts) は循環になるため
  // import せず、依存の無い sourceSignal.ts から getter だけを静的 import している
  // (以前は sources.ts を dynamic import していたが、main.ts が静的 import 済みで
  //  チャンク分割の実効が無く INEFFECTIVE_DYNAMIC_IMPORT 警告だけが出ていた)。
  const signal = getCurrentSourceSignal();
  _columnFetchInflight = api.aggregateColumns(S.CURRENT_SOURCE, { signal })
    .then((ci: any) => { S.COLUMN_INFO = ci; renderCsvColumns(); })
    .catch((e: any) => {
...
export function renderCsvColumns() {
  ...
  let columns: { name: string; samples: any[]; isNumeric: boolean }[];     // [{ name, samples: [...], isNumeric }]
  let rowCount: number;
  if (S.RAW && S.RAW.length) {
    ...
    columns = names.map(col => {
      ...
      const isNumeric = samples.length > 0 && samples.slice(0, 10).every((v: any) => !isNaN(Number(v)) && v !== '');
