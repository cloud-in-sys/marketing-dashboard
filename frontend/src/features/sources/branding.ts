...
function imgReady(img: HTMLImageElement | null): Promise<unknown> {
  if (!img) return Promise.resolve();
  if (img.complete && img.naturalWidth > 0) return Promise.resolve();
  if (typeof img.decode === 'function') return img.decode().catch(() => {});
  // resolve をそのままリスナーとして渡しているので、解決値は Event になる。
  // Promise<void> と書くと型が合わない (元コードの挙動どおり unknown のままにする)。
...
function waitForBrandImages() {
  ...
  const imgs = [
    document.querySelector('#login-brand-logo img') as HTMLImageElement | null,
    document.querySelector('#brand-logo img') as HTMLImageElement | null,
  ];
...
export function applyBranding(data: any) {
  ...
  const makeLogoHtml = (imgCls: string, tintedCls: string, fallbackCls: string, wrapCls: string, tintColor: string) => {
  ...
  if (faviconUrl) {
    const link = (existing || Object.assign(document.createElement('link'), { rel: 'icon' })) as HTMLLinkElement;
