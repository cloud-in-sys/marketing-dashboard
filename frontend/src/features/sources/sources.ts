...
import { emit, on } from '@app/events.ts';
import { S, switchSource, saveSheetsInput, loadSheetsInput,
  saveBqInput, loadBqInput,
  saveSourceMethod,
  clearSourceRaw } from '@app/state.ts';
import { escapeHtml } from '@shared/utils/utils.ts';
import { parseCSV } from '@shared/utils/csv.ts';
import { showModal } from '@shared/ui/modal.ts';
import { populateFilters, renderFilters, renderMSDynamic, closeFloatingMs } from '@filters/index.ts';
import { renderViewNav, highlightActiveView, renderCustomTabs } from '../presets/tabs.ts';
import { initTabStates, loadTabState, renderPresets, renderTabPresetSelect } from '../presets/presets.ts';
import { exitSettingsMode, renderCsvColumns, confirmDiscardUnsavedChanges } from '../settings/settings.ts';
import { api } from '@api/index.ts';
import { extractSpreadsheetId } from './sheets.ts';
import { getCurrentSourceSignal, startNewSourceSignal } from './sourceSignal.ts';
import { makeSortable } from '@shared/ui/sortable.ts';
import { hasPerm } from '@app/auth.ts';
import { FEATURES, dlog, UPDATER_SA } from '@app/config.ts';
import { getOptions } from '@shared/utils/utils.ts';
import { invalidateAggregateCache, abortInFlightAggregate } from '@aggregate/aggregateBackend.ts';
...
let _syncUrl: () => void = () => {};
export function setSyncUrl(fn: () => void) { if (typeof fn === 'function') _syncUrl = fn; }
...
export function renderSourceNav() {
  ...
  const list = document.getElementById('source-nav') as any;
  if (list) {
    const canManage = hasPerm('manageSources');
    list.innerHTML = S.DATA_SOURCES.map((ds: any) => {
...
// ドロップダウンの開閉
function toggleSourceDropdown(force?: boolean) {
...
document.addEventListener('click', e => {
  ...
  if (!(e.target as HTMLElement).closest('#source-dropdown')) toggleSourceDropdown(false);
});
// Escキーで閉じる
...
document.getElementById('source-rename-btn')?.addEventListener('click', async () => {
  ...
  try {
    ...
    ds.name = newName;
    document.getElementById('source-view-title')!.textContent = newName as string;
    renderSourceNav();
  } catch (err: any) {
...
document.getElementById('source-nav')!.addEventListener('click', e => {
  const del = (e.target as HTMLElement).closest('[data-del-source]') as HTMLElement | null;
  if (del) {
  ...
  const rename = (e.target as HTMLElement).closest('[data-rename-source]') as HTMLElement | null;
  if (rename) {
...
export async function enterSourceView(options: { skipGuard?: boolean } = {}): Promise<boolean> {
...
// 更新はサービスアカウント (SA) が行うため、対象データへの SA 共有を促す固定ノート。
// SA メールは app-config (UPDATER_SA / per-tenant) から読む。ハードコードしない。
// 空なら汎用文にフォールバック。SA メールは XSS 防止に escapeHtml を通す。
// 更新用 SA のメール。backend (ランタイム SA を metadata から取得) を優先し、
// 無ければ app-config の UPDATER_SA、それも無ければ空 (汎用文)。一度取得したらキャッシュ。
let _updaterSa: string | null | undefined;  // undefined=未取得
async function resolveUpdaterSa(): Promise<string> {
  if (_updaterSa === undefined) {
    try { _updaterSa = (await api.getUpdaterServiceAccount()).serviceAccount || null; }
    catch { _updaterSa = null; }
  }
  return _updaterSa || UPDATER_SA || '';
}

async function renderSaShareNote(method: string) {
  const el = document.getElementById(method === 'sheets' ? 'sheets-connector-note' : 'bq-connector-note');
  if (!el) return;
  const sa = escapeHtml(await resolveUpdaterSa());
  if (method === 'sheets') {
    el.innerHTML = sa
      ? `更新はサービスアカウントが自動で行います。このスプレッドシートを <strong>${sa}</strong> に「閲覧者」で共有してください。`
      : '更新はサービスアカウントが自動で行います。このスプレッドシートを更新用サービスアカウントに「閲覧者」で共有してください。';
  } else {
    el.innerHTML = sa
      ? `BigQuery で更新用サービスアカウント <strong>${sa}</strong> にクエリ実行・データ閲覧の権限を付与してください。`
      : 'BigQuery で更新用サービスアカウントにクエリ実行・データ閲覧の権限を付与してください。';
  }
}

function renderSnapshotMeta(method: string, meta: any) {
  const el = document.getElementById(method === 'sheets' ? 'sheets-snapshot-meta' : 'bq-snapshot-meta');
  if (!el) return;
  if (!meta?.exists) {
    el.innerHTML = 'まだスナップショットがありません。「今すぐ更新」を押して取得してください。';
    return;
  }
  const when = formatRelativeTime(meta.updatedAt);
  el.innerHTML = `最終更新: ${escapeHtml(when)}  (${(meta.rows || 0).toLocaleString()}行)`;
}
...
function isFresh(sid: string, version: number): boolean {
  return S.CURRENT_SOURCE === sid && sourceLoadVersion === version;
}

// 各 multi フィルタの選択肢を populate (バックエンド or S.RAW から)。
async function populateFilterOptionsFor(sid: string, version: number, signal?: AbortSignal) {
...
export async function loadSnapshotIfNeeded(options: { emitRender?: boolean } = {}): Promise<string> {
  ...
  const signal = startNewSourceSignal();
...
// このデータソースのメソッド + 入力をクリアする (別メソッドへ切り替える時)。
...
async function refreshSnapshotNow(method: string) {
  const sid = S.CURRENT_SOURCE;
  if (!sid) return;
  const fetchBtn = document.getElementById(method === 'sheets' ? 'sheets-fetch-btn' : 'bq-fetch-btn') as HTMLButtonElement | null;
...
let _sourceViewColInfoInflight: Promise<any> | null = null;
...
function renderSourceView() {
  ...
  document.querySelectorAll<HTMLElement>('.source-method-card').forEach(c => {
  ...
  let columns: any[] | undefined;          // [{ name, samples, isNumeric }]
  let previewRows: any[] | undefined;      // 配列
  if (hasRows) {
    columns = Object.keys(rows[0]).map(col => {
      ...
      const isNumeric = vals.slice(0, 10).every((v: any) => !isNaN(Number(v)) && v !== '');
    ...
    const maxRows = Math.max(0, ...columns!.map((c: any) => (c.samples || []).length));
    previewRows = [];
  ...
  if (columns?.length) {
    ...
    colEl.innerHTML = columns.map((col: any) => {
      const kind = col.isNumeric ? '数値' : '文字列';
      const sampleHtml = (col.samples || []).length
        ? (col.samples || []).map((v: any) => `<span>${escapeHtml(String(v).slice(0, 30))}</span>`).join(' / ')
        : '';
    ...
    if (previewRows?.length) {
      ...
      previewEl.innerHTML = `<div class="source-preview-wrap"><table class="source-preview-table">
        ...
        <tbody>${previewRows.map((r: any) => `<tr>${colNames.map((c: string) => `<td>${escapeHtml(String(r[c] ?? ''))}</td>`).join('')}</tr>`).join('')}</tbody>
      </table></div>`;
    } else {
  ...
  // Sheets UI state。更新は SA (ADC) で行うため OAuth ゲートは無い。
  // form (URL/シート名の入力欄 + 保存/今すぐ更新/解除) は常に表示し、SA 共有案内を出す。
  const sheetsForm = document.getElementById('sheets-form')!;
  {
    const savedInput = loadSheetsInput();
    (document.getElementById('sheets-url-input') as HTMLInputElement).value = savedInput.url || '';
    (document.getElementById('sheets-tab-input') as HTMLInputElement).value = savedInput.tab || '';
  }
  sheetsForm.classList.remove('hidden');
  renderSaShareNote('sheets');

  // BQ UI state
  const bqForm = document.getElementById('bq-form')!;
  {
    const savedBq = loadBqInput();
    (document.getElementById('bq-project-input') as HTMLInputElement).value = savedBq.project || '';
    (document.getElementById('bq-query-input') as HTMLInputElement).value = savedBq.query || '';
  }
  bqForm.classList.remove('hidden');
  renderSaShareNote('bq');
}
...
async function confirmOverwriteData(newMethod: string): Promise<boolean> {
  const existing = S.SOURCE_DATA[S.CURRENT_SOURCE!] || [];
  if (existing.length === 0) return true;
  const methodLabel = ({ csv: 'CSV', sheets: 'Googleスプレッドシート', bq: 'BigQuery' } as Record<string, string>)[newMethod] || newMethod;
...
document.getElementById('source-file')!.addEventListener('change', async e => {
  ...
  if (!f) return;
  if (!(await confirmOverwriteData('csv'))) { (e.target as HTMLInputElement).value = ''; return; }
...
document.querySelectorAll('.source-method-card').forEach(card => {
  card.addEventListener('click', async e => {
    ...
    if ((e.target as HTMLElement).closest('input,button,label.file-btn')) return;

    const targetMethod = (card as HTMLElement).dataset.method;
    const ds = S.DATA_SOURCES.find((d) => d.id === S.CURRENT_SOURCE);
    const currentMethod = ds?.method || '';

    // 既に別メソッドに連携済みなら切替をブロック
    if (currentMethod && currentMethod !== targetMethod) {
      const labels: Record<string, string> = { csv: 'CSVアップロード', sheets: 'Google スプレッドシート', bq: 'BigQuery' };
      await showModal({
        title: '連携を変更できません',
...
document.getElementById('sheets-fetch-btn')!.addEventListener('click', async () => {
  const urlOrId = (document.getElementById('sheets-url-input') as HTMLInputElement).value.trim();
  const fileId = extractSpreadsheetId(urlOrId);
  const tab = (document.getElementById('sheets-tab-input') as HTMLInputElement).value.trim();
...
// ----- SHEETS: 保存 (設定だけ更新。次回の定期更新から反映。スナップショットの再取得はしない) -----
document.getElementById('sheets-save-btn')!.addEventListener('click', async () => {
  const url = (document.getElementById('sheets-url-input') as HTMLInputElement).value.trim();
  const tab = (document.getElementById('sheets-tab-input') as HTMLInputElement).value.trim();
  ...
  try {
    ...
    _clearSheetsDirty();
    await showModal({title: '保存しました', body: '設定を保存しました。次回の定期更新から反映されます（今すぐ反映するには「今すぐ更新」を実行してください）。', okText: 'OK', cancelText: ''});
  } catch (e: any) {
...
document.getElementById('bq-fetch-btn')!.addEventListener('click', async () => {
  const project = (document.getElementById('bq-project-input') as HTMLInputElement).value.trim();
  const query = (document.getElementById('bq-query-input') as HTMLTextAreaElement).value.trim();
...
document.getElementById('add-source')!.addEventListener('click', async () => {
  ...
  const copyFromId = (document.getElementById('copy-source-select') as HTMLSelectElement | null)?.value || '';
  try {
    ...
    // 作成直後にサイドバーへ反映 (switchSource が stale 等で return しても、作られたことが分かる)。
    renderSourceNav();
    // 作成は明示アクションなので skipGuard で確実に新ソースへ遷移する (白紙作成と同じ挙動)。
    // コピー作成は config/presets のロードが重く、guard/stale で無言 return して
    // 「飛ばない」ことがあったため。
    const switched = await switchSource(created.id, { skipGuard: true });
    if (!switched) return;
    reloadFullUI();
  } catch (e: any) {
