...
import type { CardConfig } from '@app/models.ts';
import { api } from '@api/index.ts';
import { FEATURES, dlog } from '@app/config.ts';
import { setBackendCache, dimsKeyFor } from './aggregateCache.ts';
import type { BackendCacheEntry, FilterSpec } from './aggregateCache.ts';
import { hasVisibleSparklineMetric } from '@features/dashboard/table/sparkline.ts';
import { resolveDateFilter } from '@filters/dateFilter.ts';

/**
 * prefetchAggregates の戻り値。
 * ok:false のとき呼び出し側 (main.js render) はエラーバナーを出してレンダラを中止する責務がある。
 * aborted は「新しい render に追い越された」だけなのでエラー扱いしない。
 */
export type PrefetchResult =
  | { ok: true; skipped?: true; cached?: true; aborted?: true; error?: undefined }
  // 失敗時に aborted/cached/skipped が付くことは無い。`?: undefined` と書くことで
  // ok を絞る前でも `result.aborted` を読めるようにしつつ、判別可能ユニオンは保つ
  // (main.ts は ok より先に aborted を見る。読めないと as any が必要になる)。
  | { ok: false; error: string; skipped?: undefined; cached?: undefined; aborted?: undefined };
...
export function serializeFilters(): FilterSpec[] {
  ...
  for (const def of defs) {
    ...
    if (def.type === 'date_range') {
      ...
      if (v instanceof Set && v.size > 0) {
        ...
        const sorted = [...v].map((x: any) => String(x)).sort();
...
function computeCardMonthFilter(card: CardConfig): FilterSpec | null {
...
function yesterdayMonth(offset = 0): string {
  const d = new Date();
...
function monthSubtract(yyyymm: string, offset: number): string {
...
const keyedCache = new Map<string, { cache: BackendCacheEntry; expiresAt: number }>();    // cacheKey -> { cache, expiresAt }
const inflightByKey = new Map<string, Promise<BackendCacheEntry>>(); // cacheKey -> Promise<cache>

function keyedCacheGet(key: string): BackendCacheEntry | null {
...
function keyedCacheSet(key: string, cache: BackendCacheEntry) {
...
function collectPrefetchDimCombos(): string[][] {
  const seen = new Map<string, string[]>(); // key -> dims[]
  const add = (dims: string[]) => { const k = dimsKeyFor(dims); if (!seen.has(k)) seen.set(k, dims); };
  add([]);  // totals
...
export async function prefetchAggregates(rows: any[]): Promise<PrefetchResult> {
  ...
  // CURRENT_VIEW は null になり得る。null 索引は undefined なので ?. がそのまま効く。
  ...
  const cardJobs: { cardId: any; monthFilter: FilterSpec }[] = [];
