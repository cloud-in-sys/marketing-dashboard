import { S, DEFAULT_BASE_FORMULAS, DEFAULT_FORMULAS } from '@app/state.ts';
import { getBackendTotals } from './aggregateCache.ts';
...
// ===== 型 =====
// where 句の右辺。today() は評価時に基準日からのオフセットで解決する。
export type WhereValue =
  | { kind: 'literal'; value: string }
  | { kind: 'today'; offset: number };

// where 句の AST。`false` は「パースエラー」を表す番兵で、null (フィルタなし) とは別物。
// この 2 つを取り違えると「壊れた式が全行一致になる」ので、型の上でも区別しておく。
export type AstNode =
  | { type: 'clause'; field: string; op: string; value: WhereValue }
  | { type: 'and'; left: AstNode; right: AstNode }
  | { type: 'or'; left: AstNode; right: AstNode };

/** 集計関数 1 つ分の仕様。column が null なのは count() のみ */
...
/** 式から集計呼び出しを placeholder に括り出した結果 */
export interface LiftResult {
  /** 集計部分を __agg_N__ に置換した残りの算術式 */
  lifted: string;
  /** placeholder -> 集計仕様 */
...
function parseClause(clause: string): { field: string; op: string; valueStr: string } | null {
...
const liftCache = new Map<string, LiftResult>();          // formula -> { lifted, specs }
const compiledLiftedCache = new Map<string, (ctx: any) => number>(); // lifted -> compiled fn
let cachedBaseKeys: string[] | null = null;
let cachedDerivedKeys: string[] | null = null;
let cachedMetricDefsRef: any[] | null = null;

function ensureKeyCache() {
  if (cachedMetricDefsRef !== S.METRIC_DEFS) {
    cachedMetricDefsRef = S.METRIC_DEFS;
    cachedBaseKeys = S.METRIC_DEFS.filter((m) => m.type === 'base').map((m: any) => m.key);
    cachedDerivedKeys = S.METRIC_DEFS.filter((m) => m.type === 'derived').map((m: any) => m.key);
  }
}

export function baseMetricKeys(): string[] { ensureKeyCache(); return cachedBaseKeys!; }
export function derivedMetricKeys(): string[] { ensureKeyCache(); return cachedDerivedKeys!; }
...
function resolveValue(val: WhereValue, today: string): string {
  if (val.kind === 'today') return addDays(today, val.offset);
  return val.value;
}
function normMaybeDate(s: any): string {
  if (s == null) return '';
  const str = String(s);
...
function evalFilter(cell: any, op: string, target: any): boolean {
  // 文字列ベースの演算子(数値化しない)
...
function parseValue(s: string): WhereValue {
  s = s.trim();
...
function findCloseParen(str: string, start: number): number {
  let depth = 1;
...
function parseAggregateInner(fnName: string, inner: string): AggSpec | null {
  inner = inner.trim();
  const wm = /\bwhere\b/i.exec(inner);
  let column: string | null, whereStr: string;
  if (wm) {
...
function tokenizeWhere(str: string): Token[] {
  const tokens: Token[] = [];
  let i = 0;
  let buf = '';
  const flush = () => {
...
function parseWhereAst(tokens: Token[]): AstNode | false {
  let pos = 0;
  const peek = (): Token | undefined => tokens[pos];
  const eat = () => tokens[pos++];
  function orExpr(): AstNode | false {
    let left = andExpr();
  ...
  function andExpr(): AstNode | false {
    let left = atom();
    if (left === false) return false;
...
function parseWhereStr(whereStr: string): AstNode | null | false {
...
// AST ノードを行データに対して評価
function evalAst(node: AstNode | null, row: any, today: string): boolean {
  if (!node) return true;
...
export function liftFormula(formula: string): LiftResult {
  ...
  let nextId = 0;
  const specs: Record<string, AggSpec> = {};
  let result = '';
  let lastEnd = 0;
  ...
  if (tw) {
    ...
    const whereStr = tw[2];
    const ast = parseWhereStr(whereStr);
    if (ast && (ast as unknown) !== false && specs[ph] && specs[ph].filters == null) {
...
function computeAggregate(rows: any[], spec: AggSpec, today: string): number {
  const { fn, column, filters } = spec;
  // filters は AST (or null = フィルタなし)
  const ast = filters || null;
  let s = 0, count = 0, minV: number | null = null, maxV: number | null = null, distinct: Set<any> | null = null;
...
export function parseBaseFormula(formula: any) {
...
export function isPureBaseFormula(formula: any): boolean {
...
function compileLifted(formula: string): (ctx: any) => number {
  ...
  try {
    ...
    const fn = new Function('ctx', `"use strict"; try{var v=(${code});return v===v&&v!==1/0&&v!==-1/0?v:0}catch(e){return 0}`) as (ctx: any) => number;
...
export function evalFormula(formula: any, ctx: any): number {
  const { lifted } = liftFormula(String(formula || ''));
  return compileLifted(lifted)(ctx);
}

const _aggregateCache = new WeakMap<object, { ref: any; today: string; result: Record<string, number> }>();

export function aggregate(rows: any[]): Record<string, number> {
  ...
  const evalKey = (key: string, formula: any) => {
