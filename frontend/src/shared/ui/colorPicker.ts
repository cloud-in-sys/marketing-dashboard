...
class DashboardColorPicker extends HTMLElement {
  // すべて declare。フィールド宣言に初期化子を書くと (useDefineForClassFields により)
  // コンストラクタで defineProperty が走って実行時の挙動が変わるため、型だけにしている。
  declare _open: boolean;
  declare _built: boolean;
  declare _suppressEvent: boolean;
  declare _docClickHandler: (e: MouseEvent) => void;
  declare _scrollHandler: () => void;
  declare _swatch: HTMLButtonElement;
  declare _popover: HTMLElement;
  /** vanilla-colorful の <hex-color-picker>。型定義は無いので any */
  declare _picker: any;
  declare _hexInput: HTMLInputElement;
  ...
  constructor() {
    ...
    this._docClickHandler = (e: MouseEvent) => {
      if (!this.contains(e.target as Node) && this._popover && !this._popover.contains(e.target as Node) && this._open) this._close();
    };
  ...
  _build() {
    ...
    `;
    this._swatch = this.querySelector('.dcp-swatch') as HTMLButtonElement;
    this._popover = this.querySelector('.dcp-popover') as HTMLElement;
    this._picker = this.querySelector('hex-color-picker');
    this._hexInput = this.querySelector('.dcp-hex') as HTMLInputElement;

    // クリックは custom element 全体で拾う。popover 内のクリックは開閉対象外。
    this.addEventListener('click', (e) => {
      if (this._popover && this._popover.contains(e.target as Node)) return;
    ...
    this._picker.addEventListener('color-changed', (e: CustomEvent) => {
  ...
  _setValue(v: string, fromUser = false) {
