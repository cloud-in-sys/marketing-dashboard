// ===== Drag-reorder helper =====
// e.target は EventTarget なので closest() を持たない。実際には常に Element なので、
// 型の上だけ HTMLElement として扱う (実行時の挙動は変えていない)。
export function makeSortable(
  container: HTMLElement,
  onReorder: (dragKey: string, overKey: string, before: boolean) => void,
) {
  ...
  container.addEventListener('mousedown', e => { mousedownTarget = e.target as HTMLElement; }, true);
  container.addEventListener('dragstart', e => {
    const item = (e.target as HTMLElement).closest('[data-drag-key]') as HTMLElement | null;
    ...
    if (previewEl) {
      try { e.dataTransfer!.setDragImage(previewEl as Element, 10, 10); } catch (_) {}
    }
  });
  ...
  container.addEventListener('dragover', e => {
    ...
    const over = (e.target as HTMLElement).closest('[data-drag-key]') as HTMLElement | null;
