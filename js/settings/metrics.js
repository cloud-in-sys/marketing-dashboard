...
// 英語エラー文を日本語化
function _translateError(raw) {
  if (!raw) return raw;
  let msg = String(raw)
    .replace(/^[\w.[\]]+\s+has syntax error:\s*/, '構文エラー: ')
    .replace(/^[\w.[\]]+\s+contains forbidden token:\s*/, '禁止された語が含まれています: ')
    .replace(/^[\w.[\]]+\s+too long.*$/, '式が長すぎます (最大 2000 文字)')
    .replace(/^[\w.[\]]+\s+must be a string$/, '式は文字列で指定してください')
    .replace(/^Invalid expression:\s*/i, '');
  const map = [
    [/missing \) after argument list/i,    '閉じ括弧 ) が足りません (関数呼び出し)'],
    [/missing \) in parenthetical/i,       '閉じ括弧 ) が足りません'],
    [/missing \} after property list/i,    '閉じ括弧 } が足りません (オブジェクトリテラル)'],
    [/missing \] after element list/i,     '閉じ括弧 ] が足りません (配列リテラル)'],
    [/Unexpected end of input/i,           '式が途中で終わっています'],
    [/Invalid or unexpected token/i,       '不正な記号が含まれています'],
    [/Unexpected identifier/i,             '予期しない識別子があります'],
    [/missing : after property id/i,       'オブジェクトの : が足りません'],
    [/Unterminated string literal/i,       '文字列リテラルが閉じられていません'],
    [/Unterminated template literal/i,     'テンプレートリテラルは使用できません'],
  ];
  for (const [re, ja] of map) if (re.test(msg)) return ja;
  const m = /Unexpected token\s+'?([^']+?)'?$/.exec(msg);
  if (m) return `予期しない記号: ${m[1]}`;
  return msg;
}
function _showFormulaError(idx, msg) {
  const input = document.querySelector(`[data-def-formula="${idx}"]`);
  const errEl = document.querySelector(`[data-def-formula-error="${idx}"]`);
...
function _scheduleFormulaCheck(idx, key, type, src) {
  const t = _checkTimers.get(idx);
  if (t) clearTimeout(t);
  _checkTimers.set(idx, setTimeout(() => {
    _checkTimers.delete(idx);
    const label = type === 'base' ? `baseFormulas.${key}` : `formulas.${key}`;
    _showFormulaError(idx, _validateFormula(src, label));
  }, 250));
}
...
export function renderMetricsDoc() {
  ...
  const renderRow = (m, i) => {
    ...
    return `<div class="metrics-doc-row" data-def-idx="${i}" data-drag-key="${i}" draggable="true">
      ...
      <div class="metric-formula-error" data-def-formula-error="${i}" hidden></div>
    </div>`;
  };
  ...
  el.querySelectorAll('.metrics-doc-row[data-def-idx]').forEach(row => {
    ...
    // 既存の式に構文エラーがあれば初期表示時点でハイライト
    if (def) {
      const src = def.type === 'base'
        ? (baseSrc[def.key] || DEFAULT_BASE_FORMULAS[def.key] || '')
        : (derivedSrc[def.key] || DEFAULT_FORMULAS[def.key] || '');
      const label = def.type === 'base' ? `baseFormulas.${def.key}` : `formulas.${def.key}`;
      _showFormulaError(idx, _validateFormula(src, label));
    }
  });
}
...
export function setupMetricsEvents() {
  ...
  document.getElementById('metrics-doc').addEventListener('input', e => {
    ...
    if (e.target.matches('[data-def-label]')) {
      ...
      const k = def.key;
      const idx = e.target.dataset.defFormula;
      ...
      _scheduleFormulaCheck(idx, k, def.type, v);
    }
    markMetricsDirty();
  });
  ...
  document.getElementById('metrics-save-btn').addEventListener('click', async () => {
    ...
    // backend と同じ validation を保存前に走らせる (PATCH 後にエラーで戻されないように)
    for (let i = 0; i < defs.length; i++) {
      const d = defs[i];
      const src = d.type === 'base'
        ? ((S.METRICS_DRAFT_BASE || S.BASE_FORMULAS)[d.key] || '')
        : ((S.METRICS_DRAFT || S.METRIC_FORMULAS)[d.key] || '');
      const label = d.type === 'base' ? `baseFormulas.${d.key}` : `formulas.${d.key}`;
      const err = _validateFormula(src, label);
      if (err) {
        _showFormulaError(i, err);
        await showModal({title: '保存できません', body: `「${d.label || d.key}」の計算式にエラーがあります:\n\n${err}`, okText: 'OK', cancelText: ''});
        return;
      }
    }
    for (const d of defs) {
...
CTR / CPC / CPM 等の割り算は派生型にして、基礎メトリクスを参照する式 (例: clicks / impression) にしてください。多重ディメンションのピボット親行で値が壊れるのを防ぐためです。`,
    ...
    // PATCH 中はサイドエフェクトを伴う UI 更新はしない (体感は数百ms 増えるが古い config での集計を回避)。
    try {
      await flushConfigNow();
    } catch (e) {
      const raw = e?.message || String(e);
      const ja = _translateError(raw);
      const isValidation = /Invalid expression|syntax error|forbidden token|too long/.test(raw);
      const body = isValidation
        ? `計算式にエラーがあります:\n\n${ja}`
        : `サーバーへの保存に失敗しました。ネットワーク接続を確認してもう一度お試しください。\n\n${ja}`;
      await showModal({title: '保存に失敗しました', body, okText: 'OK', cancelText: ''});
      return;
    }
    emit('render');
